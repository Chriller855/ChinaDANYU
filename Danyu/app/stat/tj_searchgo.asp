...
<table width="500" cellspacing="0" align="center" cellpadding="0" border="0">
  ...
  <tr height="30">
	...
	<INPUT type="radio" name="overwrite" value="0" checked<%end if%> <%if name="" then%>> 同名时提示&nbsp;
	<INPUT type="radio" name="overwrite" value="1" checked<%end if%> <%if name<>"" then%>> 同名时覆盖
...
for i=0 to tuhow-1
	if not rs.EOF then
		alt(i)=rs(thewhich)
		val(i)=rs("theval")
		if clng(val(i))>themax then themax=clng(val(i))
		thesum=thesum + clng(val(i))
		rs.MoveNext
	else
		alt(i)=""
		val(i)=0
...
for i= 0 to tuhow-1
%>
  <td width="<%=tuwidth%>" align=center><a	title="<%=alt(i)%><%=unit%>，访问<%=val(i)%>次，<%=theper(i)%>%"><font
